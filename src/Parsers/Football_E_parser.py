        ...
        for index, row in data.iterrows():
            ...
            for i in range(1, 12):
                if self.fetch_safety_data(row["home_player_"+str(i)], int) is not None:
                    list_joueur_home_id.append(
                        self.fetch_safety_data(row["home_player_"+str(i)], int)
                    )
                if self.fetch_safety_data(row["away_player_"+str(i)], int) is not None:
                    list_joueur_home_id.append(
                        self.fetch_safety_data(row["away_player_"+str(i)], int)
                    )
            for joueur_home in list_joueur_home_id:
                self.dict_equipe[
                    self.fetch_safety_data(row["home_team_api_id"], int)
                    ].ajouter_joueur(self.dict_player[joueur_home])
